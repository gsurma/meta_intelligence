	...
	def select_elite(self, population):
		...
		for i in range(0, len(population)):
			...
			if score == self.max_fitness_score:
				...
				print("\nFOUND SOLUTION: " + chromosome + " for: " + repr(self.target) +  " and it took: " + str(int((current_time-self.start_time)/60)) + " minutes")
		...
		result = brainfuck.evaluate(chromosome)
		best_fitness_score = self.fitness_string(result, self.target)
